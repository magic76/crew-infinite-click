...
for(const required of ['tapJuice:{lastAt:0,streak:0,lastFrenzyAt:0,heat:0,jackpots:0}','localReleaseFeedback','state.fx.tapAccent(px,py,streak)','state.fx.tapFrenzyAccent(px,py,streak,heat)','playClick(mood,.28+power*.30,Math.max(streak,Math.round(promiseProgress*20)))']){
