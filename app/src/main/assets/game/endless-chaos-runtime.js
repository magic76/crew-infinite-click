(() => {
  ...
  const VERSION="ENDLESS_CHAOS_V27";
  ...
  const state={
    ...
    stage:1,phase:"BOOT",recipe:null,startedAt:0,minEndAt:0,endAt:0,climaxUntil:0,nextAt:0,
    baseline:null,progress:0,chaos:0,multiplier:1,peak:1,lastChaosAt:0,realityUsed:false,
    holding:false,lastRapidAt:0,lastWorldFxAt:0,lastObjectFxAt:0,lastPetFxAt:0,lastDiagAt:0,lastDiag:null,
    shots:[],particles:[],rings:[],lightning:[],flashes:[],shakeUntil:0,shakePower:0,bannerUntil:0,lastFrame:now()
  };

  function recipeFor(stage){
    const n=Math.max(1,Math.floor(Number(stage)||1)),mega=n%5===0,world=(n-1)%4;
    return {stage:n,mega,world,weapon:WEAPONS[(n-1)%4],weapon2:mega?WEAPONS[n%4]:null,worldMod:WORLD_MODS[world],petMod:PET_MODS[Math.floor((n-1)/2)%4],objectMod:OBJECT_MODS[(n+1)%4],climax:mega?"REALITY_BREAK":CLIMAXES[(n-1)%4],target:mega?Math.min(44,25+Math.floor(n*.5)):Math.min(30,11+Math.floor(n*.65)),minMs:mega?12500:9000,maxMs:mega?20500:15500+(n%3)*1200};
  }
  ...
  function createOverlay(){
    const c=document.createElement("canvas");c.id="endless-chaos-v27-overlay";c.style.cssText="position:fixed;inset:0;width:100%;height:100%;pointer-events:none;z-index:24;";document.body.appendChild(c);state.canvas=c;state.ctx=c.getContext("2d");resize();addEventListener("resize",resize,{passive:true});
    const h=document.createElement("div");h.id="endless-chaos-v27-hud";h.style.cssText="position:fixed;left:50%;top:calc(env(safe-area-inset-top,0px) + 10px);transform:translateX(-50%);width:min(88vw,420px);z-index:25;pointer-events:none;color:#fff;font-family:system-ui,-apple-system,sans-serif;text-shadow:0 2px 10px rgba(0,0,0,.8);";
    h.innerHTML='<div data-stage style="font-size:13px;font-weight:950;letter-spacing:1.6px;text-align:center"></div><div style="display:flex;justify-content:space-between;gap:12px;align-items:end;margin-top:2px"><div data-recipe style="font-size:10px;font-weight:750;opacity:.88;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"></div><div data-chaos style="font-size:20px;font-weight:1000;white-space:nowrap"></div></div><div style="height:7px;margin-top:6px;border-radius:999px;background:rgba(9,12,24,.60);box-shadow:inset 0 0 0 1px rgba(255,255,255,.15)"><div data-progress style="height:100%;width:0;border-radius:999px;background:linear-gradient(90deg,#82e8ff,#ffe172,#ff87c9);box-shadow:0 0 14px rgba(160,225,255,.65)"></div></div><div data-banner style="position:fixed;left:50%;top:23vh;transform:translate(-50%,-50%) scale(.90);opacity:0;transition:opacity .14s ease,transform .18s ease;white-space:nowrap;text-align:center;font-size:29px;font-weight:1000;letter-spacing:1px;text-shadow:0 4px 24px rgba(0,0,0,.86)"></div>';
    document.body.appendChild(h);state.hud={root:h,stage:h.querySelector("[data-stage]"),recipe:h.querySelector("[data-recipe]"),chaos:h.querySelector("[data-chaos]"),progress:h.querySelector("[data-progress]"),banner:h.querySelector("[data-banner]")};
  }
  ...
  function startStage(n,immediate){
    const t=now(),r=recipeFor(n),d=diag();state.stage=n;state.recipe=r;state.phase="RUN";state.startedAt=t;state.minEndAt=t+r.minMs;state.endAt=t+r.maxMs;state.climaxUntil=0;state.nextAt=0;state.baseline=baselineFrom(d);state.progress=0;state.realityUsed=false;state.lastWorldFxAt=0;state.lastObjectFxAt=0;state.lastPetFxAt=0;
    if(r.mega){state.chaos=Math.max(state.chaos,42);registerChaos(0);showBanner(`MEGA STAGE ${n}`,`${r.weapon} + ${r.weapon2} · ${r.climax}`,1300);megaEntryFx();}else showBanner(`STAGE ${n}`,`${r.weapon} · ${r.worldMod} · ${r.petMod}`,850);
    updateHud(t);
  }
  ...
  function registerChaos(amount){state.chaos=clamp(state.chaos+Math.max(0,Number(amount)||0),0,100);state.lastChaosAt=now();state.multiplier=Math.max(1,Math.round(1+state.chaos*.99));state.peak=Math.max(state.peak,state.multiplier);if(state.chaos>=75&&!state.realityUsed){state.realityUsed=true;realityBreak(.82);}}
  ...
  function consumeDiagnostics(d){
    const prev=state.lastDiag;if(!d){return;}if(prev){const bp=Math.max(0,(d.buttonPresses||0)-(prev.buttonPresses||0)),hits=Math.max(0,(d.directHits||0)-(prev.directHits||0)),objs=Math.max(0,(d.objectHits||0)-(prev.objectHits||0)),shots=Math.max(0,(d.buttonShots||0)-(prev.buttonShots||0));if(bp)registerChaos(bp*1.25);if(hits)registerChaos(hits*4.1);if(objs)registerChaos(objs*2.6);if(shots)registerChaos(Math.min(3,shots*.34));}
    state.lastDiag=d;
  }

  function updateStage(dt,t,d){
    if(state.phase==="BOOT"){startStage(1,true);return;}if(t-state.lastChaosAt>650){state.chaos=Math.max(0,state.chaos-dt*(state.recipe&&state.recipe.mega?.0028:.0048));state.multiplier=Math.max(1,Math.round(1+state.chaos*.99));}
    state.progress=readProgress(d);stageModifiers(t,d);
    if(state.phase==="RUN"&&((state.progress>=1&&t>=state.minEndAt)||t>=state.endAt))climax(t);
    else if(state.phase==="CLIMAX"&&t>=state.climaxUntil){state.phase="CLEAR";state.nextAt=t+650;registerChaos(state.recipe.mega?14:7);showBanner(state.recipe.mega?"MEGA CLEAR":"STAGE CLEAR",`CHAOS ×${state.multiplier}`,650);burst(innerWidth*.5,innerHeight*.40,state.recipe.mega?42:24,WORLD_COLORS[state.recipe.world]);}
    else if(state.phase==="CLEAR"&&t>=state.nextAt)startStage(state.stage+1,false);
  }

  function stageModifiers(t,d){
    const r=state.recipe;if(!r||state.phase==="CLEAR")return;const ps=d&&d.pets||[];
    if(t-state.lastWorldFxAt>(r.mega?150:260)){state.lastWorldFxAt=t;if(r.worldMod==="STORM"&&Math.random()<.75)lightning(Math.random()*innerWidth,60+Math.random()*innerHeight*.58,1);else if(r.worldMod==="BUBBLE_RAIN")for(let i=0;i<2;i++)bubble(Math.random()*innerWidth,innerHeight+20,8+Math.random()*10);else if(r.worldMod==="LOW_GRAVITY"&&Math.random()<.65)spark(Math.random()*innerWidth,innerHeight*.25+Math.random()*innerHeight*.55,"#b9efff",-25-Math.random()*30);else if(r.worldMod==="CARNIVAL_FRENZY"&&Math.random()<.72)star(Math.random()*innerWidth,Math.random()*innerHeight*.68,WORLD_COLORS[r.world],1);}
  ...
  function climax(t){const r=state.recipe;if(state.phase!=="RUN")return;state.phase="CLIMAX";state.climaxUntil=t+(r.mega?2800:2100);state.progress=1;registerChaos(r.mega?24:12);showBanner(r.mega?"REALITY BREAK":r.climax,`STAGE ${r.stage} CLIMAX`,r.mega?1500:1000);if(r.climax==="THUNDER_FINALE")thunderFinale();else if(r.climax==="PINBALL_BREAK")pinballBreak();else if(r.climax==="WORLD_BREAK")worldBreak();else if(r.climax==="OVERDRIVE_RAIN")overdriveRain();else realityBreak(1);}
  ...
  function stageWeapon(x,y,auto){const r=state.recipe;if(!r)return;const origin={x:innerWidth*.5,y:innerHeight-Math.max(78,innerHeight*.08)};if(r.weapon==="MULTISHOT"||r.weapon2==="MULTISHOT")fireMulti(origin,r.mega?5:3);if(r.weapon==="RICOCHET"||r.weapon2==="RICOCHET")fireRicochet(origin,r.mega?4:3);if(r.weapon==="LIGHTNING"||r.weapon2==="LIGHTNING")lightningToPet();if(r.weapon==="RAPID"||r.weapon2==="RAPID"){if(!auto)fireMulti(origin,2);}if(state.multiplier>=40&&!auto)fireMulti(origin,state.multiplier>=75?3:1);}

  function fireMulti(origin,count){const ps=pets();if(!ps.length)return;for(let i=0;i<count;i++){const p=ps[(Math.floor(Math.random()*ps.length)+i)%ps.length];shot(origin,{x:p.x||innerWidth*.5,y:p.y||innerHeight*.38},WORLD_COLORS[state.recipe.world],i*38);}}
  function fireRicochet(origin,count){const ps=pets();if(!ps.length)return;const pts=[];for(let i=0;i<count;i++){const p=ps[(i+Math.floor(Math.random()*ps.length))%ps.length];pts.push({x:p.x||innerWidth*.5,y:p.y||innerHeight*.42});}chainShot(origin,pts,WORLD_COLORS[state.recipe.world]);}
  function lightningToPet(){const ps=pets(),p=ps.length?ps[Math.floor(Math.random()*ps.length)]:null;lightning(p&&p.x||innerWidth*(.2+Math.random()*.6),p&&p.y||innerHeight*.42,1.1);}

  function shot(a,b,color,delay){state.shots.push({kind:"shot",a:{...a},b:{...b},color,delay:(delay||0)/100,t:0,d:.22,arc:(Math.random()<.5?-1:1)*(18+Math.random()*36)});}
  function chainShot(a,points,color){let start={...a};points.forEach((p,i)=>{state.shots.push({kind:"shot",a:{...start},b:{...p},color,delay:i*.09,t:0,d:.18,arc:(Math.random()<.5?-1:1)*(14+Math.random()*22),impact:i===points.length-1?1:.45});start={...p};});}

  function updateShots(dt){for(let i=state.shots.length-1;i>=0;i--){const s=state.shots[i];if(s.delay>0){s.delay-=dt/1000;continue;}s.t+=dt/1000;const q=clamp(s.t/s.d,0,1);if(q>=1){burst(s.b.x,s.b.y,8,s.color);ring(s.b.x,s.b.y,12,s.color,.65);state.shots.splice(i,1);}}}

  function drawShots(c){for(const s of state.shots){if(s.delay>0)continue;const q=clamp(s.t/s.d,0,1),e=1-Math.pow(1-q,3),x=s.a.x+(s.b.x-s.a.x)*e,y=s.a.y+(s.b.y-s.a.y)*e+Math.sin(Math.PI*q)*s.arc*(1-q*.2),angle=Math.atan2(s.b.y-s.a.y,s.b.x-s.a.x);c.save();c.translate(x,y);c.rotate(angle);c.shadowBlur=22;c.shadowColor=s.color;c.globalAlpha=.9;c.strokeStyle=s.color;c.lineWidth=9;c.beginPath();c.moveTo(-42,0);c.lineTo(-5,0);c.stroke();c.strokeStyle="#fff";c.lineWidth=3;c.beginPath();c.moveTo(-26,0);c.lineTo(-2,0);c.stroke();c.fillStyle="#fff";c.beginPath();c.arc(0,0,7,0,Math.PI*2);c.fill();c.restore();}}
  ...
  function updateHud(t){const h=state.hud,r=state.recipe;if(!h||!r)return;const until=state.phase==="RUN"?state.endAt:(state.phase==="CLIMAX"?state.climaxUntil:state.nextAt),sec=Math.max(0,Math.ceil((until-t)/1000));h.stage.textContent=`${r.mega?"MEGA ":""}STAGE ${r.stage} · ${sec}s`;h.recipe.textContent=[r.weapon,r.weapon2,r.worldMod,r.petMod].filter(Boolean).join(" · ");h.chaos.textContent=`CHAOS ×${state.multiplier}`;h.chaos.style.color=state.multiplier>=75?"#fff":state.multiplier>=40?"#ffe177":"#c9f5ff";h.progress.style.width=`${Math.round(state.progress*100)}%`;h.progress.style.filter=state.multiplier>=50?"brightness(1.4) drop-shadow(0 0 8px #fff)":"";if(state.bannerUntil&&t>=state.bannerUntil){h.banner.style.opacity="0";h.banner.style.transform="translate(-50%,-50%) scale(.94)";state.bannerUntil=0;}}

  function loop(t){const dt=Math.min(50,t-state.lastFrame||16.7);state.lastFrame=t;const d=diag();consumeDiagnostics(d);updateStage(dt,t,d);if(state.holding&&state.recipe&&(state.recipe.weapon==="RAPID"||state.recipe.weapon2==="RAPID")&&t-state.lastRapidAt>90){state.lastRapidAt=t;stageWeapon(innerWidth*.5,innerHeight-80,true);}updateShots(dt);updateParticles(dt);updateHud(t);shakeGame(t);render();requestAnimationFrame(loop);}

  function init(){if(state.ready)return;const d=diag();if(!d){setTimeout(init,120);return;}state.ready=true;createOverlay();captureGameCanvas();state.lastDiag=d;document.addEventListener("pointerdown",onDown,true);document.addEventListener("pointerup",onUp,true);document.addEventListener("pointercancel",onUp,true);startStage(1,true);requestAnimationFrame(loop);try{window.AndroidGame&&AndroidGame.onRendererReady&&AndroidGame.onRendererReady(VERSION);}catch(_){} }

  window.EndlessChaosV27={version:VERSION,recipeFor,diagnostics:()=>({stage:state.stage,phase:state.phase,recipe:state.recipe,progress:state.progress,chaos:state.chaos,multiplier:state.multiplier,peak:state.peak}),forceStage:n=>startStage(n,false)};
  init();
})();
